...
const STATE_LITERALS = new Set([
	...
	// Merchant OS internal tab / route tokens
	'Overview', 'Business', 'Market', 'Transactions', 'Settings', 'Messages', 'Members',
	'Wallets', 'Staff', 'Dashboard', 'Programs', 'Assets', 'Operations', 'Financials',
	'System', 'Validator Management', 'Card Issuance Setup', 'MembersLoyalty',
])

function lineHasInternalRouteToken(line, english) {
	if (!/(handleTabChange|activeTab|navChromeTab|setActiveTab|transactionsSidebar|navigate\(|pathname|location\.)/.test(line)) {
		return false
	}
	const e = escapeRegExp(english)
	return new RegExp(`['"]${e}['"]`).test(line)
}
...
function wireFile(filePath) {
	...
	for (let li = 0; li < lines.length; li++) {
		...
		for (const [english, key] of enEntries) {
			...
			if (lineHasInternalRouteToken(line, english)) continue
