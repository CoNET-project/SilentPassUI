...
import { AlertTriangle, ArrowLeft, ArrowRight, Check, Clock, Calendar, Copy, Gift, Loader2, RefreshCw, Ticket } from 'lucide-react'
...
export function ActiveCouponTicketItem({
	...
	const claimButton = showActionButton ? (
		<div className="pointer-events-auto absolute right-6 top-1/2 z-[2] -translate-y-1/2 sm:right-8">
			{usesRedeemedStatusCapsule ? (
				<span
					className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-slate-400/50 bg-slate-100/90 shadow-sm ring-1 ring-slate-300/40 backdrop-blur-sm dark:border-slate-500/50 dark:bg-slate-800/90 dark:ring-slate-600/30"
					aria-label={claimActionAriaLabel}
					title={tu('redeemed')}
				>
					<Ticket className="h-4 w-4 text-slate-500 dark:text-slate-300" strokeWidth={2.25} aria-hidden />
				</span>
			) : usesClaimedStatusCapsule ? (
				<span
