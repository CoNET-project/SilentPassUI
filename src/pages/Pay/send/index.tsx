...
export default function PayScreen ({close, beamioer, preferredToAddress, mode = 'eoa-pay', aaAccountUsdcBalance, focusAmountOnMount, onShowFuelCenter, initialNote, initialSendAmount}: Props) {
		...
		return (
			<div className="flex-1 px-5 pt-6 pb-8 flex flex-col items-center justify-center
				...
				<div className="text-xs text-slate-500 dark:text-slate-400 mb-4">
					{/cashcode/i.test(messageData?.sginTatle) ? tu('share_this_beamio_cashcode_as_a_link_qr_or_redeem_code') : tu('it_may_take_a_few_seconds_to_appear_on_chain') } 
				</div>
	...
	const onPay = async () => {
		...
		if (isAaEoaTransfer && transferDirection === 'aa-to-eoa') {
			...
			const aaBal = Number(aaAccountUsdcBalance ?? 0)
			if (amount > aaBal) {
				setSendError(tu('insufficient_balance'))
				return
			}
			setProcessing(true)
			...
			try {
				...
				if (!fromChain || !fromChain.startsWith('0x')) {
					setSendError(tu('no_express_pay_found_please_create_or_link_a_express_pay_first'))
					setProcessing(false)
					return
				}
				if (myAddress && fromChain.toLowerCase() === myAddress.toLowerCase()) {
					setSendError(tu('express_pay_address_cannot_be_the_same_as_your_eoa_please_create_or_link'))
					setProcessing(false)
					return
				}
				aaAccount = fromChain
			} catch (e: any) {
				setSendError(e?.message ?? tu('failed_to_get_express_pay_address'))
				setProcessing(false)
				return
			}
			...
			try {
				...
				if (payerEOA) {
					...
					if (total < 2) {
						setBunitError(`B-Unit 不足：付款方需要 2 B-Unit 转账费（余额：${total} B-Units)`)
						setProcessing(false)
						return
					}
				}
			} catch (e) {
				setSendError((e as Error)?.message ?? tu('b_unit_balance_check_failed'))
				setProcessing(false)
				return
			}
			try {
				...
				if (!currentCurrency || !String(currentCurrency).trim()) {
					setSendError(tu('currency_is_required_for_accounting'))
					setProcessing(false)
					return
				}
				...
				if (result.success && result.USDC_tx) {
					...
					setSendError(result.error || tu('aatoeoa_failed'))
				}
			} catch (e: any) {
				setProcessing(false)
				setSendError(e?.message || tu('aatoeoa_request_failed'))
			}
			return
		}
		...
		try {
			...
			const receiveName = item?.username ?? (toAddress === profiles?.[0]?.aaAccount ? tu('express_pay') : shortAddress(toAddress))
	...
	<div className="relative">
		...
		<div className="">
			...
			<div className="flex-1 px-5 pt-6 pb-8 flex flex-col items-center justify-center bg-transparent text-inherit">
				...
				<p className="mt-4 text-lg font-semibold text-slate-800 dark:text-slate-100 text-center">{tu('insufficient_b_units')}</p>
				...
				(
					...
					<div className={cardCreate ? "opacity-0 pointer-events-none select-none" : ""}>
						...
						<>
							<section className="input">
								<AmountCurrency
									...
									showLimit={0}
									sendError={insufficientAaBalance ? tu('insufficient_balance') : sendError}
							...
							{isPayeeAA ? (
								<section className="mt-4 flex justify-center">
									<div className="inline-flex items-center gap-2 rounded-full bg-violet-100 dark:bg-violet-900/40 border border-violet-200 dark:border-violet-700/60 px-4 py-2.5">
										...
										<span className="font-semibold text-violet-700 dark:text-violet-300">{tu('smart_routing_analysis')}</span>
									</div>
								</section>
							) : (
							<section className="mt-4 rounded-2xl bg-white dark:bg-slate-800/50 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700 overflow-hidden">
								<div className="flex items-center justify-between px-4 pt-3 pb-2">
									<span className="text-[11px] font-medium tracking-wider text-slate-500 dark:text-slate-400 uppercase">{tu('paying_from')}</span>
									{isAaEoaTransfer && (
										<button
											type="button"
											onClick={() => setTransferDirection(d => d === 'eoa-to-aa' ? 'aa-to-eoa' : 'eoa-to-aa')}
									...
									<div className="flex items-center gap-3 min-w-0">
										...
										<div className="min-w-0">
											<div className="font-semibold text-slate-900 dark:text-slate-100 truncate">
												{isAaEoaTransfer && transferDirection === 'aa-to-eoa' ? tu('express_pay') : tu('main_vault')}
											</div>
						...
						<div className="mt-6 flex gap-3 w-full">
							...
							>
								{message
									? (sendError ? tu('try_again') : tu('confirm_sign'))
