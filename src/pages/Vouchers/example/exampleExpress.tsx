...
type Tx = {
	...
	isMixed: boolean
	route: RouteItem[]
	fees: { gas: number; service: number; bUnits: number; gasBUnits: number; minting?: number }
	hashes: { base: string | string[] | null; conet: string | null }
	meta?: { requestAmount?: number; expiresAt?: string; via?: string; originalRequestId?: string; issuer?: string }
};
...
const TRANSACTIONS: Tx[] = [
 ...
 },
 {
   ...
   meta: { issuer: 'Starbucks' } // 增加发行方标识
 },
 {
   id: 'tx_020',
   type: 'reload_card',
   title: 'Buy CashTrees Card', // 修正标题
   handle: '@cashtrees_official',
   ...
   isMixed: true,
   route: [
     { asset: '$CTree', amount: 50.00, type: 'Voucher', symbol: 'pts', source: "" }, // 🔥 完美修正：获得 50 $CTree
     { asset: 'USDC', amount: 36.50, type: 'Cash', symbol: '$', source: 'Main Wallet (EOA)' } // 使用了 36.50 USDC
   ],
   fees: { gas: 0, service: 0, bUnits: 0, gasBUnits: 0, minting: 99 }, // 注入 99 铸造费
   hashes: { base: '0x5a1...c2d3', conet: '0x8e4...f5a6' },
   meta: { issuer: 'CashTrees' } // 注入发行方标识
 },
 {
   id: 'tx_021',
   type: 'upgrade_card',
   title: 'Upgrade to Black Card',
   handle: '@cashtrees_official',
   ...
   isMixed: true,
   route: [
     { asset: '$CTree-Black', amount: 300.00, type: 'Voucher', symbol: 'pts', source: "" },
     { asset: '$CTree', amount: -50.00, type: 'Voucher', symbol: 'pts', source: "Merged" }, // 🔥 对齐基础资产 $CTree 被合并销毁
     { asset: 'USDC', amount: 182.50, type: 'Cash', symbol: '$', source: 'Main Wallet (EOA)' }
   ],
   fees: { gas: 0, service: 0, bUnits: 0, gasBUnits: 0, minting: 99 }, // 注入 99 铸造费
   hashes: { base: '0x7b2...d4e5', conet: '0x9c1...a3b4' },
   meta: { issuer: 'CashTrees' } // 注入发行方标识
 },
 {
   id: 'tx_010',
...
export default function BeamioTransactions({ initialTab = 'All' }: { initialTab?: string }) {
 ...
 const getDisplayAmount = (tx: Tx, tab: string) => {
 ...
 // 详情页底部聚合结算引擎
 const getSettledString = (tx: Tx): string | null => {
   ...
   // 针对升级合并的特殊排版：旧资产 + 现金 -> 新资产
   if (tx.type === 'upgrade_card') {
      const newVoucher = tx.route.find((r: RouteItem) => r.type === 'Voucher' && r.amount > 0);
      const oldVoucher = tx.route.find((r: RouteItem) => r.type === 'Voucher' && r.amount < 0);
      const cash = tx.route.find((r: RouteItem) => r.type === 'Cash');
      if (newVoucher && oldVoucher && cash) {
          return `${Math.abs(oldVoucher.amount).toFixed(2)} ${oldVoucher.asset} + ${Math.abs(cash.amount).toFixed(2)} ${cash.asset} → ${newVoucher.amount.toFixed(2)} ${newVoucher.asset}`;
      }
   }
 ...
 // 动态预言机汇率引擎
 const renderExchangeRate = (tx: Tx) => {
   if (tx.amountFiat === 0 || tx.amountUSDC === 0 || tx.type === 'request_create' || tx.type === 'request_expired' || tx.type === 'request_canceled') return null;
  
   let usdcRate = 1;
   // 获取第一个正向 Voucher，作为汇率锚定目标
   const voucherRoute = tx.route?.find((r: RouteItem) => r.type === 'Voucher' && r.amount > 0);
  
   if (tx.type === 'reload_card' || tx.type === 'upgrade_card') {
   ...
   return (
     <div className="flex justify-between items-start text-[14px]">
        ...
        <div className="flex flex-col items-end gap-1">
          {voucherRoute && (
            <span className="font-semibold text-black">1 {tx.currencyFiat} = 1 {voucherRoute.asset.replace('-Black', '').replace('-Green', '')}</span>
          )}
 ...
 TRANSACTIONS.forEach((tx: Tx) => {
   ...
   if (activeTab === 'All') {
     ...
     else if (tx.type === 'reload_card' || tx.type === 'upgrade_card') amtPrimary = formatCurrency(-Math.abs(tx.amountFiat), tx.currencyFiat); // 上帝视角看到的是法币购买支出
   ...
   tx.route.forEach((r: RouteItem, idx: number) => {
     const isMainWallet = r.source?.includes(tu('main_wallet')) ?? false;
     ...
     } else if (tx.type === 'upgrade_card') {
       // 升级逻辑：Cash tab 展示出钱那部分，Express tab 展示进账新卡部分，忽略被合并的旧卡项(在宏观列表隐藏细节)
       if (activeTab === 'Cash' && r.type === 'Cash') {
           belongsToTab = true;
           isPos = false;
           fiatValue = Math.abs(tx.amountFiat); // 法币扣了250CAD
       } else if (activeTab === 'Express' && r.type === 'Voucher' && r.amount > 0) {
           belongsToTab = true;
           isPos = true;
           fiatValue = r.amount; // 获得了 300的黑卡
       }
     } else {
       if (activeTab === 'Cash') {
             ...
             >
               <div className="flex items-center gap-4">
                 ...
                 <div className={`w-[48px] h-[48px] rounded-[14px] flex items-center justify-center shadow-sm relative overflow-hidden ${
                   ...
                   (tx.type === 'reload_card' || tx.type === 'upgrade_card') ? 'bg-[#AF52DE]/10 text-[#AF52DE]' :
                   ...
                   {(tx.type === 'reload_card' || tx.type === 'upgrade_card') && <CreditCard size={22} strokeWidth={2} />}
                 ...
                 <div className="flex flex-col gap-0.5">
                   ...
                   <div className="flex items-center gap-1.5">
                     ...
                     {/* Status Badges */}
                     {tx.isMixed && tx.route?.length > 1 && tx.type !== 'upgrade_card' && (
                     ...
                     {tx.type === 'upgrade_card' && (
                       <div className="flex items-center gap-0.5 px-1.5 py-0.5 bg-[#AF52DE]/10 rounded-md">
                         <ArrowRightLeft size={10} className="text-[#AF52DE]" />
                         <span className="text-[10px] font-bold text-[#AF52DE] uppercase tracking-wider">Merged</span>
                       </div>
                     )}
           ...
           >
             ...
             <div className="text-center mb-8">
               <div className={`w-[72px] h-[72px] mx-auto rounded-[24px] flex items-center justify-center shadow-lg mb-5 ${
                 ...
                 (selectedTx.type === 'reload_card' || selectedTx.type === 'upgrade_card') ? 'bg-[#AF52DE] text-white shadow-purple-200' :
                 ...
                 'bg-[#AF52DE] text-white'
                  ...
                  {(selectedTx.type === 'reload_card' || selectedTx.type === 'upgrade_card') && <CreditCard size={36} strokeWidth={1.5} />}
               ...
               <h2 className={`text-[28px] font-bold tracking-tight leading-tight ${(selectedTx.type === 'request_expired' || selectedTx.type === 'request_canceled') ? 'text-gray-400' : 'text-black'}`}>
                  {selectedTx.type === 'request_create' || selectedTx.type === 'request_expired' || selectedTx.type === 'request_canceled'
                    ...
                    : formatCurrency(selectedTx.type === 'reload_card' || selectedTx.type === 'upgrade_card' ? selectedTx.amountFiat : selectedTx.amountFiat, selectedTx.currencyFiat)
                  }
               </h2>
               ...
               <div className="bg-gradient-to-b from-[#F9FAFB] to-white border border-gray-100 p-5 rounded-[24px] mb-6 shadow-sm">
                  <div className="flex justify-between items-center mb-5">
                   <h3 className="text-[15px] font-bold text-black flex items-center gap-2">
                     <div className={`w-6 h-6 rounded-full flex items-center justify-center ${selectedTx.type === 'upgrade_card' ? 'bg-[#AF52DE]/20' : 'bg-[#FF9500]/20'}`}>
                       {selectedTx.type === 'upgrade_card' ? <ArrowRightLeft size={14} className="text-[#AF52DE]" /> : <Zap size={14} className="text-[#FF9500] fill-[#FF9500]" />}
                     </div>
                     {selectedTx.type === 'upgrade_card' ? tu('asset_merge_upgrade') : tu('smart_routing')}
                   </h3>
                 ...
                 <div className="space-y-4 relative">
                   ...
                   {selectedTx.route.map((item, index) => {
                     // 🔥 修正：智能判断当前条目的正负极性
                     let isItemPos = selectedTx.amountUSDC > 0;
                     if (selectedTx.type === 'reload_card') {
                        isItemPos = item.type === 'Voucher'; // 买卡时，Voucher 是进 (+)，Cash 是出 (-)
                     } else if (selectedTx.type === 'upgrade_card') {
                        isItemPos = item.type === 'Voucher' && item.amount > 0; // 升级时，新卡是进 (+)，旧卡和Cash是出 (-)
                     }


                     // 🔥 修正：根据资产类型动态分配专属颜色
                     let iconBg = 'bg-[#1562f0] text-white'; // USDC 默认蓝色
                     if (item.type === 'Voucher') {
                        if (item.asset.includes('Black')) iconBg = 'bg-black text-white'; // 黑卡专属颜色
                        else if (item.asset.includes('Green') || item.asset === '$CTree') iconBg = 'bg-[#34C759] text-white'; // 绿卡专属颜色
                        else iconBg = 'bg-[#AF52DE] text-white'; // 其他 Voucher 默认紫色
                     }
                     ...
                     return (
                       <div key={index} className="flex justify-between items-center">
                         <div className="flex items-center gap-4">
                            <div className={`w-5 h-5 rounded-full border-[3px] border-white shadow-sm flex items-center justify-center text-[9px] font-bold z-10 ${iconBg}`}>
                              {item.symbol}
                            </div>
                         ...
                         <span className={`text-[15px] font-semibold ${isItemPos ? 'text-[#34C759]' : 'text-black'}`}>
                           {isItemPos ? '+' : '-'}{Number.isInteger(item.amount * 100) ? Math.abs(item.amount).toFixed(2) : Math.abs(item.amount).toFixed(4)}
                         </span>
                       </div>
                     );
                   })}
             ...
             <div className="bg-[#F9FAFB] rounded-[24px] p-5 space-y-4 mb-8">
                ...
                {/* 🔥 彻底重构的费用面板：纯 C 端被动场景 (买卡/升级/核销) 完全隐藏，只对主动消耗或收款方显示 */}
                {!(selectedTx.type === 'reload_card' || selectedTx.type === 'upgrade_card' || selectedTx.type === 'voucher_burn') && (
                  <>
                    <div className="flex justify-between items-center text-[14px]">
                    ...
                    <div className="flex justify-between items-center text-[14px]">
                       <span className="text-gray-500 font-medium">Minting Fee</span>
                       <span className="font-bold text-gray-600 bg-gray-100 px-2 py-0.5 rounded text-[12px] flex items-center gap-1">
             ...
             <div className="space-y-3 mb-8">
                ...
                {Array.isArray(selectedTx.hashes.base) ? (
                   selectedTx.hashes.base.map((hash, index) => (
