...
export default function BeamioTransactions({ initialTab = '全部' }: { initialTab?: string }) {
 ...
 TRANSACTIONS.forEach((tx) => {
   ...
   if (tx.accountType === 'Internal') {
     if (activeTab === '全部' || activeTab === 'Cash' || activeTab === 'Express') {
   ...
   tx.route.forEach((r: RouteItem, idx: number) => {
     const isMainWallet = (r.source ?? '').includes('主钱包');
         ...
         <div className="mt-3 flex justify-center">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/60 backdrop-blur-sm border border-white/40 shadow-sm">
              ...
              <span className="text-[11px] font-medium text-gray-500">
                {activeTab === 'Cash' ? 'Main Wallet (EOA)' : activeTab === 'Express' ? 'Express Pay (AA)' : '全部账户'}
              </span>
            </div>
         </div>
       </div>
                    ...
                    <>
                       <div className={`rounded-[20px] p-5 text-center border ${selectedTx.status === '待处理' ? 'bg-[#1562f0]/5 border-[#1562f0]/20' : 'bg-[#FF9500]/5 border-[#FF9500]/20'}`}>
                           <p className={`text-[15px] font-medium mb-4 ${selectedTx.status === '待处理' ? 'text-[#1562f0]' : 'text-[#FF9500]'}`}>
                             {selectedTx.status === '待处理' ? '订单已锁定，有人正在结账…' : '收款码有效，等待付款。'}
                           </p>
                           <button disabled={selectedTx.status === '待处理'} className={`w-full py-3.5 bg-white border rounded-[14px] font-bold text-[15px] flex items-center justify-center gap-2 shadow-sm transition-transform ${selectedTx.status === '待处理' ? 'text-gray-400 border-gray-200 opacity-50' : 'text-[#FF9500] border-[#FF9500]/30 active:scale-95'}`}>
                             <Share2 size={18} /> {selectedTx.status === '待处理' ? '已锁定' : '再次分享'}
                           </button>
                       </div>
                ...
                <div className="flex justify-between items-center text-[14px]">
                   <span className="text-gray-500 font-medium">
                     {(selectedTx.type === 'request_create' || selectedTx.type === 'request_canceled' || selectedTx.type === 'request_expired') ? 'Request Title' :
                      ...
                      selectedTx.type === 'transfer_in' ? '收款自' :
                   ...
                   <div className="flex items-center justify-between p-3.5 bg-gray-50 border border-gray-200 rounded-[16px] border-dashed opacity-70">
                       <div className="flex items-center gap-2.5">
                         ...
                         <span className="text-[13px] font-semibold text-gray-500">
                            {(selectedTx.status === '待处理' || selectedTx.status === 'Canceled') ? 'Base L2（已锁定）' : 'Base L2（待处理）'}
                         </span>
                       </div>
                       <span className="text-[11px] font-medium text-gray-400">
                          {selectedTx.status === '待处理' ? '结账进行中' : selectedTx.status === 'Canceled' ? '请求已撤回' : '等待付款'}
                       </span>
                   </div>
                )}
