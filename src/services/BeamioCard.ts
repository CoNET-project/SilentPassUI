...
export function isReferralMerchantStartKitCode(code: string): boolean {
...
export async function queryReferralMerchantStartKitRedeemOnChain(
	...
	const c = new ethers.Contract(CONET_REFERRAL_REGISTRY_VAULT_V1, REFERRAL_MERCHANT_CODE_ABI, conetDepinProvider)
	try {
		const r = await c.merchantCodes!(codeHash)
		const issuerL0 = r.issuerL0 as string
		const paid = (r.paidBunitAmount ?? 0n) as bigint
		const validAfter = Number(r.validAfter ?? 0n)
		const validBefore = Number(r.validBefore ?? 0n)
		const active = Boolean(r.active)
		const claimed = Boolean(r.claimed)
		const cancelled = Boolean(await c.merchantCodeCancelled!(codeHash).catch(() => false))
		const timeOk =
			(validAfter === 0 || now >= validAfter) && (validBefore === 0 || now <= validBefore)
		let error: string | undefined
		if (!active || cancelled) error = 'Redeem is not active'
		...
		else if (!issuerL0 || issuerL0 === ethers.ZeroAddress) error = 'Invalid Start Kit issuer'
		const redeemable = active && !claimed && !cancelled && timeOk && paid > 0n
		return {
			valid: true,
			codeHash,
		...
		return {
			...
			timeOk: false,
			redeemable: false,
			error: err?.shortMessage ?? err?.message ?? 'merchantCodes failed',
		}
	}
}

/** Gasless claim of ReferralRegistry merchant Start Kit via Cluster → Master `claimMerchantCodeFor`. */
export async function postReferralMerchantStartKitClaim(params: {
	...
	try {
		const wallet = new ethers.Wallet(params.privateKeyArmor.trim())
		const redeemHash = ethers.keccak256(ethers.toUtf8Bytes(secret))
		const pre = await queryReferralMerchantStartKitRedeemOnChain(secret)
		if (!pre.redeemable) return { success: false, error: pre.error ?? 'This code cannot be redeemed.' }
		const nonceRes = await fetch(
			`${beamioApi}/api/referralRegistryClaimNonce?account=${encodeURIComponent(wallet.address)}`
		)
