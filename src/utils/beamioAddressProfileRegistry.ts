...
/** Aligns with `BeamioCapsule` / `searchUsername` peer shape */
...
const BIZ_LS_PREFIX = 'beamio:biz-example:';

export function beamioAddressProfileMapKey(walletPartitionLower: string): string {
  return `${BIZ_LS_PREFIX}eoa:${walletPartitionLower}:address-profiles:v1`;
}
...
/** Pick best peer from `searchUsername` JSON (same heuristics as biz Transactions / Members). */
...
export function recordFromSearchPeer(addrLower: string, peer: ReturnType<typeof pickPeerFromSearchUsernameResponse>): BeamioAddressProfileRecord | null {
  if (!peer) return null;
  const has =
...
export function toBeamioCapsuleItem(
  r: BeamioAddressProfileRecord | undefined | null
): {
  first_name?: string;
  firstName?: string;
  last_name?: string;
