...
const summaryInFlight = new Map<string, Promise<CardProgramReferrerChainSummary>>()
...
/** Chain may store EOA or AA; product UI exposes EOA. */
...
async function readCardProgramReferrerChainSummaryUncached(
	cardAddress: string,
...
export async function readCardProgramReferrerChainSummary(
	...
	const key = ethers.getAddress(cardAddress).toLowerCase()
	const existing = summaryInFlight.get(key)
	if (existing) return existing
	const p = readCardProgramReferrerChainSummaryUncached(cardAddress).finally(() => {
