/**
 * Ported from bizSite Issued Program Asset card rendering.
 * Keep gradient and foreground behavior aligned without cross-project imports.
 */

export type CardTierGradientTheme = {
	isDarkStart: boolean
	primary: string
	secondary: string
	tertiary: string
...
function normalizeBackgroundHex(input: string | undefined | null): string | null {
...
function tierBackgroundColorForPayload(raw: string | undefined | null): string | undefined {
	if (typeof raw !== 'string') return undefined
	const s = raw.trim()
	if (!s) return undefined
	const withHash = s.startsWith('#') ? s : `#${s}`
	return normalizeBackgroundHex(withHash) ?? undefined
}

function hexToRgb(hex: string): { r: number; g: number; b: number } | null {
	const n = normalizeBackgroundHex(hex)
	if (!n) return null
	const h = n.slice(1)
	return {
...
function preferDarkForegroundPerWcag(backgroundLuminance: number): boolean {
	...
	const contrastWithWhiteText = wcagContrastRatio(1, lb)
	const contrastWithBlackText = wcagContrastRatio(lb, 0)
...
function cardDiagonalGradientT(u: number, v: number, aspectWidthOverHeight = 1.6): number {
	...
	const den = a2 + 1
	return (clamp(u, 0, 1) * a2 + clamp(v, 0, 1)) / den
}

function preferDarkForegroundRightSmallTextZone(
	...
	for (const [u, v] of samples) {
		const t = cardDiagonalGradientT(u, v, aspectWidthOverHeight)
		...
		minCrWhite = Math.min(minCrWhite, wcagContrastRatio(1, lb))
		minCrBlack = Math.min(minCrBlack, wcagContrastRatio(lb, 0))
	}
...
function sameFamilyGradientEndRgb(startHex: string): { r: number; g: number; b: number } {
	...
	const { h, s: s0, v: v0 } = rgbToHsv(rgb.r, rgb.g, rgb.b)
	if (deepBackground) {
...
function tierGradientStops(backgroundColorRaw: string | undefined): { start: string; end: string } {
...
export function cardTierGradientCss(backgroundColorRaw: string | undefined): string {
	const { start, end } = tierGradientStops(backgroundColorRaw)
	return `linear-gradient(135deg, ${start} 0%, ${end} 100%)`
}

export function cardTierGradientTheme(backgroundColorRaw: string | undefined): CardTierGradientTheme {
	...
	const darkForeground = preferDarkForegroundRightSmallTextZone(startRgb, endRgb, 1.6)
	const primary = darkForeground ? '#0f172a' : '#ffffff'
	...
	const iconOrbitBg = darkForeground ? 'rgba(0,0,0,0.07)' : 'rgba(255,255,255,0.08)'
	const defaultBadgeBg = darkForeground ? 'rgba(15,23,42,0.1)' : 'rgba(255,255,255,0.22)'
	const defaultBadgeFg = darkForeground ? 'rgba(15,23,42,0.75)' : 'rgba(255,255,255,0.95)'
	return {
