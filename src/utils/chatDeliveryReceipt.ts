...
export async function postMailboxDeliveryAck(
	...
	try {
		...
		const armored = typeof postData === 'string' ? postData : String((postData as { data?: unknown })?.data ?? postData)
		...
		console.warn('[postMailboxDeliveryAck]', msg)
		return false
	}
}
...
export async function sendDeliveryReceiptToSender(args: SendDeliveryReceiptArgs): Promise<boolean> {
	...
	try {
		...
		const msg = e instanceof Error ? e.message : e
		console.warn('[sendDeliveryReceiptToSender]', msg)
		return false
	}
}
