...
async function fetchEnsureConetAaFromApi(eoa: string): Promise<string | null> {
	try {
		const res = await fetch(`${beamioApi}/api/ensureAAForEOAOnConet?eoa=${encodeURIComponent(eoa)}`)
...
export async function ensureConetAaForEoa(eoa: string): Promise<string | null> {
	...
	const task = (async (): Promise<string | null> => {
		const existing = await resolveBeamioAaOnConet(conetProvider, norm).catch(() => null)
		if (existing) return existing

		const created = await fetchEnsureConetAaFromApi(norm)
		if (!created) return null

		const code = await conetProvider.getCode(created).catch(() => '0x')
