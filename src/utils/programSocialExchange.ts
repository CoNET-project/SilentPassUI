...
export type SocialExchangeDraft = {
	...
	/** Human Reward PT amount; metadata stores 6-decimal raw units. */
	pointsCost: string
...
const REWARD_PT_SCALE = 1_000_000

function parseRewardPtHumanToRaw(raw: unknown): number | null {
	const text = String(raw ?? '').trim()
	if (!/^\d+(?:\.\d{1,2})?$/.test(text)) return null
	const human = Number(text)
	const rawUnits = Math.round(human * REWARD_PT_SCALE)
	if (!Number.isFinite(human) || human <= 0 || !Number.isSafeInteger(rawUnits)) return null
	return rawUnits
}

function formatRewardPtRawToHuman(raw: unknown): string {
	const n = Number(raw)
	if (!Number.isFinite(n) || n <= 0) return ''
	// Legacy metadata stored whole human PT values (for example 1 or 10).
	if (Number.isSafeInteger(n) && n < 10_000) return n.toFixed(2)
	return (n / REWARD_PT_SCALE).toFixed(2)
}
...
export function normalizeSocialExchangePayload(
	...
	const points = Number(raw.pointsCost ?? raw.points_cost ?? raw.points13)
...
export function socialExchangeDraftFromMetadata(
	...
	return {
		...
		pointsCost: formatRewardPtRawToHuman(exchange.pointsCost) || String(exchange.pointsCost ?? 10),
...
export function validateSocialExchangeDraft(draft: SocialExchangeDraft): string {
	...
	const points = parseRewardPtHumanToRaw(draft.pointsCost)
	if (points == null) return 'Reward PT must be a positive amount with up to 2 decimal places.'
	if (points > 1_000_000_000_000) return 'Reward PT amount is too large.'
...
export function socialExchangeSummaryLabel(exchange: ShareTokenMetadataSocialExchange): string {
	const pts = formatRewardPtRawToHuman(exchange.pointsCost) || '0.00'
	...
	return `Use ${pts} Reward PT → 1 activity coupon`
}
